...
var (
	...
	condReady = apis.Condition{
		Type:   AzsbConditionReady,
		Status: corev1.ConditionTrue,
	}
)

// Check that AzsbSource implements the Conditions duck type.
var _ = duck.VerifyType(&AzsbSource{}, &duckv1.Conditions{})

func TestAzsbSourceStatusGetCondition(t *testing.T) {
	tests := []struct {
		name      string
		s         *AzsbSourceStatus
		...
		name:      "uninitialized",
		s:         &AzsbSourceStatus{},
		condQuery: AzsbConditionReady,
		want:      nil,
	}, {
		name: "initialized",
		s: func() *AzsbSourceStatus {
			s := &AzsbSourceStatus{}

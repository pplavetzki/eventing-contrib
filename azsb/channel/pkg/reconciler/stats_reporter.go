...
const (
	// AZServicebusChannelReadyCountN is the number of az service bus channels that have become ready.
	AZServicebusChannelReadyCountN = "as_servicebus_channel_ready_count"
	// AZServicebusChannelReadyLatencyN is the time it takes for a kafka channel to become ready since the resource is created.
	AZServicebusChannelReadyLatencyN = "as_servicebus_ready_latency"
)

var (
	// KindToStatKeys keys to do stats on
	KindToStatKeys = map[string]StatKey{
		"AZServicebusChannel": {
			ReadyCountKey:   AZServicebusChannelReadyCountN,
			ReadyLatencyKey: AZServicebusChannelReadyLatencyN,
		},
	}

	// KindToMeasurements the k8s kind to measure
